...
The seeded demonstration runs on `2026.09.10-expanded-1`. Inspections saved under an earlier pack keep the version they were judged under, as intended: a record is evidence of what the rules said on the day it was assessed.
...
Other source checks cover typography thresholds, dates of amendments, consumer-care requirements, unit-price bases, exemptions and revised-price situations. The [legal source matrix](C:/Users/offic/OneDrive/Desktop/sih/docs/LEGAL_SOURCE_MATRIX.md) records the research and official links. The bundled source pack is now `2026.09.10-expanded-1`, with source-linked applicability gates and legal corrections, and an inspector-facing description on every rule. It remains a draft requiring legal sign-off. The Rules page shows the version selected by that running server; each older inspection keeps the version originally used.
...
| Physical panel dimensions | Optional. Tula now measures the panel from the photograph, so enter these only when you want a tape measurement on the record instead of an inferred bracket — it is the more precise of the two. Both PDP width and height, in millimetres, from 1–5000 mm; the form uses a 0.1 mm step. Neither route creates a pixels-to-millimetres scale on its own. |
...
| Batch and lot markings | Extracted codes with source evidence and alternative OCR readings, including supported joined cue/code forms such as `BATCHAB1234` and the single-letter cue Indian packs often stamp, `B: 103A`. One letter is weak evidence, so the short form is accepted only with a delimiter after it and only for a code carrying both a letter and a digit — `Vitamin B: 12` and `B: 103` are not batches. A typed officer correction still requires the spelt-out cue. | Inspect dotted digits and similar-looking characters. Ambiguous cue boundaries or unsupported code formats can still require manual correction. |
...
| Allergens named on this label | Every allergen category in the FSSAI schedule, read from the ingredient text without being asked. Says whether each one also appears in a Contains statement. | A food safety referral, never a Legal Metrology finding. Read the source statement and the evidence kind before acting. |
| Allergen concerns | Explicit, possible related-ingredient and cross-contact text matches for terms you enter yourself. | Only needed for something outside the screened schedule. Read the source statement and match type. |
...
### What the label says about allergens, without being asked

**Allergens named on this label** appears on the record whenever the ingredient text names one. Nobody types anything to get it: the ingredient statement is swept against every category in the Food Safety and Standards (Labelling and Display) Regulations, 2020, regulation 5(3) — cereals containing gluten, crustacea, milk, egg, fish, peanuts, tree nuts and sulphites — plus sesame and mustard, which are Codex categories and not part of the Indian schedule.

Each one is reported with **how it was found**, and the three kinds mean different things:

| Evidence kind | What it means | Example |
|---|---|---|
| Named outright | The allergen itself is in the ingredient list. | "milk solids" in the ingredients |
| Related ingredient | An ingredient that usually implies the allergen, but not always. | "lecithin" for soy; "oats" for gluten |
| Cross-contact statement | It appears in a "may contain" or facility statement, not in the recipe. | "manufactured in a facility that contains dairy" |

The line worth an officer's attention is the one the screen draws between the two: an allergen **named outright in the ingredients but absent from any Contains statement**. That is the pattern that gets flagged, and only outright matches qualify — a referral built on a "possible" wastes somebody's afternoon.

**None of this is a finding under the Rules.** Allergen declaration is a food safety duty, not a packaged-commodities one, so it appears on the record as a referral to the food safety authority and never as an LMPCR violation, advisory or count.

**Hindi labels are screened the same way English ones are.** Rule 6 permits the declaration in Hindi in Devanagari script or in English, so the lexicon holds both: `दूध` and `milk`, `काजू` and `cashew`, `गेहूं` and `wheat` all report the same category. Two grammatical facts are handled explicitly because getting them wrong inverts the meaning: Hindi negates *after* the noun, so `ग्लूटेन रहित` is read as a free-from claim exactly as "gluten free" is; and Hindi qualifies cross-contact after the allergen too, so `मूंगफली के अंश हो सकते हैं` is a cross-contact statement, not an ingredient. The danda (`।`) ends a sentence for this purpose the way a full stop does, so a cross-contact clause cannot leak backwards onto a declaration beside it.

Devanagari recognition comes from the optional Tesseract engine with the `hin` language data. **Without it the bundled model returns boxes for every Indic glyph**, and the record says so: *"Optional Tesseract OCR is unavailable or disabled; only the RapidOCR model was used. Install Tesseract with eng and hin data for a second model and Hindi recognition."* Install it before any Hindi-language demonstration or deployment.

Measured end to end on generated Devanagari labels with known content (`python scripts/measure_hindi_allergens.py`): **3 of 4 ingredient lines were recognised, and every allergen in the lines that were recognised came back correct** — 2 of 2 recovered with the right evidence kind, zero wrong, zero spurious, and the free-from label correctly silent. The one failure was recognition, not screening: the longest conjunct-heavy line came back as `H:   3, ,  ,`. These are generated labels, not photographs of real Indian packages, and their denominators are kept apart from the real-photograph sets for that reason.

**Why there is no language model behind it.** The duty screened is a closed list, so a fixed vocabulary is the right shape for it: the screen cites the exact ingredient text it matched, returns the same answer for the same label every time, runs on the CPU with everything else, and can be argued about clause by clause. A model would add paraphrase coverage and take away all four. What the lexicon cannot do is read a language it does not know, or infer an allergen from a trade name nobody listed — and it says nothing rather than guessing in either case.

**Measured on real labels.** Seven photographs of real ingredient panels, annotated by eye before the evaluation existed: no false positives, no matches on either trap the annotations set (a serving suggestion naming eggs and butter, and a `GLUTEN FREE` badge), and four of four negative controls clean. Of five annotated allergen events, one was recovered correctly, one with the wrong evidence kind and three were lost — every failure a recognition failure rather than a screening one. The set, the annotations and the per-photograph failures are in [`data/evaluation/ingredient-panels/STATUS.md`](C:/Users/offic/OneDrive/Desktop/sih/data/evaluation/ingredient-panels/STATUS.md); reproduce with `python scripts/evaluate_ingredient_panels.py`.

To search for something the schedule does not cover, find **Ingredient and allergen review**, enter comma-separated terms and click **Check ingredient statements**. Standard concern names receive the supported matching behavior; custom ingredient names are matched literally.
...
#### Nobody has to hold a ruler against the package

Three separate things go into a height decision, and only one of them ever needed a person with a measuring tape.

**The character height** is measured from the image automatically and always was. Tula finds the connected components inside the declaration's crop and takes the upper quartile of their trimmed heights, so a comma or a speckle cannot set the answer. No input from you.

**The panel size** is now measured from the image too, where it used to require you to type width and height in millimetres. Every printed line sits on the panel, so the hull of the text that was read is a floor; if the package is wholly inside the frame, the frame is a ceiling; and where the package separates cleanly from its background, that boundary tightens the ceiling. The panel area is reported as that **bracket**, not as a single number — measured against rendered labels of known size, a point estimate at the centre of the bracket under-read the area by a median of 31% and its interval missed the truth in five cases out of eight, because where the true panel sits inside the bracket depends on how much of the frame the photographer filled. Reported as a bound the interval contained the truth in all eight. Run `python scripts/measure_panel_accuracy.py` to reproduce that. The consequence is a wide interval, and a wide interval often means Rule 8 stays inconclusive — which is the honest outcome, and better than selecting a Table I row from a number that is confidently wrong. Typing the panel dimensions is still available and still preferred: a tape against the package beats anything inferred from pixels, and it is recorded as an on-site measurement.

**The millimetre scale** is the one thing a photograph cannot supply. It is not an engineering gap that better software closes. An image records angles, not distances; the size of a thing and its distance from the lens trade off exactly, so the same picture is consistent with a small package close up and a large one further away. Recovering millimetres per pixel needs either something of known size in the frame or the camera-to-subject distance, and phones do not reliably record the latter: across the fourteen real photographs in this project's evaluation sets, **none carries an EXIF `SubjectDistance` tag and twelve carry no EXIF at all**, because uploading, downloading or forwarding an image usually strips it. That is why the scale card exists, why it is the only route to Tier B in the field, and why no distance-based estimate is offered — a number that looks like a measurement but rests on an unrecorded distance would be worse than an honest refusal.

#### Deciding a height rule without a scale, where the maths allows it

When no scale reference reached the camera, Rule 8 usually ends inconclusive. Sometimes it does not have to. Write the rule out: with `u` the panel's mean side in millimetres and `r` the ratio of character height to panel extent — a ratio a photograph *does* determine — the rule asks whether `r × u` clears the Table I minimum for a panel of area `u² / 100`. Only `u` is unknown, so Tula sweeps it across every package size the declared net quantity allows and looks at what happens across the whole range.

* Fails everywhere → the characters are short whatever the package's true size is. Reported as an **advisory** that says so and asks for a re-capture with the card.
* Holds everywhere → returning with a card would not change this rule's outcome, and the record says so, so nobody makes a second visit for nothing.
* Otherwise → silence. The answer genuinely depends on the scale, and the inconclusive verdict beside it is correct.

The direction always favours the package. A shortfall is measured against the printed text hull, the smallest the panel can be, which makes the ratio large and a shortfall harder to prove; a clearance is measured against the outer bound, which makes it small and compliance harder to prove. The size bracket rests on the declared quantity and a density prior, which is Tier C evidence, so the advisory may never become a violation — the tier gate refuses it, and it appears in the record as `SCREEN.SCALE_FREE.<rule>` alongside the statutory finding rather than in place of it.
...
## Demonstrating with the stored result

`images/` holds five photographs of one packet of Nakoda Foods Laung Sev, 400 g.
Uploading exactly those five returns a **recorded** analysis in well under a
second instead of the forty seconds recognition normally takes, so a
demonstration does not stall while an audience watches a progress bar.

The console says so, above the findings and on the saved record: the analysis
carries the fixture's name and the date it was recorded, and the record's first
note explains that the findings were reproduced from a recording rather than
recomputed. Nothing in the recording is written by hand — it is the output of
`scripts/record_demo_fixture.py`, which runs the ordinary pipeline with replay
switched off and stores whatever it decided.

Two things follow, and both are worth showing a sceptical audience:

* **Any other photograph takes the live path.** The fixture is keyed on the
  SHA-256 of every image in the set, so a judge's own packet, a re-saved copy or
  even four of these five images all run the real pipeline.
* **It can be switched off.** Start the server with `TULA_DEMO_FIXTURES=off` and
  upload the same five images to watch the live pipeline reach the same result.

A recording made under a different rule pack or recogniser is refused rather
than replayed, so it cannot quietly show yesterday's answer.

## What "confirm the product category" changes

The console's **Do this next** panel names the facts holding checks up and how
many each would release. Those counts are derived by re-running the rule pack
with the fact assumed, not from a hand-written table, so they cannot drift.

Confirming the category as **Food** is the clearest example. `LMPCR.R6.1.D.EXPIRY_FOOD`
sits at *Unverified* until then, because whether a best-before duty exists at
all is a food-law question and a packaged-commodities screen that guessed the
commodity was food would be inventing the duty it then enforces. Tick Food and
the rule decides; tick Cosmetic and it drops out of the record instead.
