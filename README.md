...
The bundled pack `2026.09.10-expanded-1` has source-linked legal corrections and applicability gates, but still requires independent legal sign-off. It differs from `2026.09.07-legal-review-1` only by carrying, on each rule, a plain description of what that rule is about and the one thing to do while it is open; no applicability condition, assertion, threshold or statutory message differs between them. The supplied PRD also describes capabilities beyond this application. See the current [implementation ledger](docs/IMPLEMENTATION_STATUS.md), the [legal source matrix](docs/LEGAL_SOURCE_MATRIX.md), and [which duties are screened and which are not](docs/RULE_COVERAGE.md).
...
**For Hindi labels, install Tesseract with the `hin` language data.** The bundled RapidOCR model recognises Latin script and digits; it returns boxes for every Devanagari glyph. Tesseract is the second recogniser that reads them, and the Rules put Hindi in Devanagari on an equal footing with English, so a deployment that will see Hindi packages needs it. Without it the application still runs and says so on every affected record rather than failing silently. On Windows install the UB Mannheim build with Hindi selected and make sure `tesseract` is on `PATH`; `TULA_TESSERACT=off` disables it and `TULA_TESSERACT_LANGS` (default `eng+hin`) selects the packs.
...
A photograph has no inherent millimetre scale, and no amount of software recovers one: an image records angles, so a small package close up and a large one further away produce the same picture. Height checks therefore need a calibrated reference in the frame — the scale card below — and without one they stay inconclusive. Across the fourteen real photographs in this project's evaluation sets, none carries an EXIF `SubjectDistance` and twelve carry no EXIF at all, which is why no distance-based estimate is offered.

The **panel size** no longer needs a tape measure. It is bracketed from the photograph itself — the hull of the text that was read is a floor, the package boundary or frame is a ceiling — and reported as that interval rather than a point estimate, because a point estimate at the bracket's centre under-read the area by a median of 31% against rendered labels of known size and missed the truth five times in eight; as a bound it covered all eight. Where no scale reached the camera at all, height rules are additionally screened for the cases the scale cannot change: a shortfall that holds at every package size the declared quantity allows is reported as a Tier C advisory, a clearance that holds throughout says a second visit with a card is unnecessary, and anything in between stays silent.

The bench supplies synthetic calibration. The CLI also accepts trusted `<image>.meta.json` sidecars; web uploads do not import calibration sidecars.
...
Two evaluations cover the ruler-free measurement and allergen work. [Panel accuracy](out/panel-accuracy/results.json) measures the bracketed panel area against rendered labels of known size across every Table I band: the reported interval contained the truth in **8 of 8**, at a median relative uncertainty of 35% and a median point error of −26%. A rendered label sits square in its frame against a clean edge, so these are a ceiling on what a shelf photograph gives. [Ingredient panels](data/evaluation/ingredient-panels/STATUS.md) scores the allergen sweep against seven real, redistributable photographs of real ingredient lists, annotated by eye before the evaluation existed: **zero false positives, zero matches on either annotated trap, four of four negative controls clean**, and of five annotated allergen events one recovered correctly, one with the wrong evidence kind and three lost — every failure a recognition failure, not a screening one. Every photograph in that set is European or North American; no comparably licensed Indian ingredient panel was found, and Open Food Facts disallows API access in its `robots.txt`.

Hindi is screened on the same footing as English, because the Rules put it there. [Devanagari measurement](out/hindi-allergen-measurement/results.json) runs generated Hindi labels of known content through real recognition and the real screen: **3 of 4 ingredient lines recognised, and every allergen in the recognised lines correct** — 2 of 2 with the right evidence kind, zero wrong, zero spurious, free-from claims correctly silent. The one failure was recognition, not screening. Devanagari needs the optional Tesseract engine with `hin` data; without it the bundled model returns boxes for every Indic glyph and the record says so explicitly. Generated labels — these denominators are kept apart from the real-photograph sets.
...
scripts/measure_panel_accuracy.py       panel area against known truth
scripts/evaluate_ingredient_panels.py   allergen sweep against real panels
scripts/measure_hindi_allergens.py      Devanagari screening end to end
